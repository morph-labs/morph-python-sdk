...
def main():
    ...
    # Example usage
    client = SemanticShellClient(
        hostname="localhost",
        username="morphvm_rwb6bu3y" + ":" + os.environ.get("MORPH_API_KEY"),
    )
    ...
    try:
        ...
        for command in commands:
            ...
            print(f"Command echo: {result.command!r}")
            print(f"Output: {result.output!r}")
            print(f"Exit code: {result.exit_code}")
            print("OSC Parameters:")
            for seq_id, params in result.osc_params.items():
                print(f"  {seq_id}:")
                print(f"    Type: {params.type.name}")
                if params.prompt_kind:
                    print(f"    Prompt kind: {params.prompt_kind.name}")
                if params.continuation:
                    print(f"    Continuation: {params.continuation.name}")
                if params.exit_code is not None:
                    print(f"    Exit code: {params.exit_code}")
                if params.aid:
                    print(f"    Activity ID: {params.aid}")
                if params.raw_params:
                    print(f"    Raw params: {params.raw_params}")
