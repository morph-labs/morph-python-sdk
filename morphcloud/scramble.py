            ...
            if i in self._resolved_positions:
                if current_control[i] > 0:
                    ...
                    if updated_control == 0:
                        # print(f"Position {i} fully resolved.")
                        pass
                else:
                    result[i] = text[i]
            else:
        ...
        with self.control_lock:
            if not all(self.control[i] <= self.control[i - 1] for i in range(1, text_len)):
                with self._print_lock:
                    ...
                    # print(f"Current control_array: {list(self.control)}")
...
def scramble_print(text: str, **config_overrides):
    ...
    try:
        ...
        for s in active_scramblers:
            s.stop()
        # print("\nAnimation interrupted!", flush=True)
    finally:
...
if __name__ == "__main__":
    ...
    COLORS = {
        "PRIMARY": "\033[38;2;220;220;204m",  # Light beige for primary text
        "HIGHLIGHT": "\033[38;2;249;193;72m",  # Vibrant gold for highlights
        "TEXT": "\033[38;2;117;182;206m",  # Soft teal for main text
        "SECONDARY": "\033[38;2;181;137;0m",  # Golden brown for secondary text
