...
class SimpleAgent:
    def __init__(self, api_key: str, runtime: Optional[Runtime] = None):
        ...
        if self.runtime:
            self.tools = self.runtime.actions.as_anthropic_tools()  # Fixed: use actions property
        else:
            self.tools = []
    ...
    def run(self, prompt: str) -> str:
        ...
        while True:
            ...
            if last_content.type == "text":
                self.messages.append({"role": "assistant", "content": last_content.text})
            ...
            if last_content.type == "tool_use":
                ...
                self.messages.append({"role": "assistant", "content": "Unexpected response type"})
