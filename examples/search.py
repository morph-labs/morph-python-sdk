...
with Runtime.create(
    ...
    response = agent.run("Could you please clone the repository https://github.com/maxvonhippel/AttackerSynthesis.git into your current workspace?")
    print(response)
    ...
    response = agent.run("Great! Now let's find a function that chooses a bit name. After that, please find all references to that function in the codebase and synthesize all your insights.")
    print(response)
    
    response = agent.run("What assumptions does the function make about the input and output of the function?")
    print(response)
...
with Runtime.create(snapshot_id=snapshot_id) as runtime:
    ...
    search_result = runtime.execute.semantic_search(
        query="a function that selects a bit name",
        max_results=10
    )
